...
import { createFilesContext, extractCurrentContext, extractPropertiesFromMessage, simplifycodeweaverActions } from './utils';
...
export async function selectContext(props: {
  ...
  const processedMessages = messages.map((message) => {
    if (message.role === 'user') {
      ...
      content = simplifycodeweaverActions(content);

      content = content.replace(/<div class=\\"__codeweaverThought__\\">.*?<\/div>/s, '');
...
---
  ...
  if (totalFiles == 0) {
    throw new Error(`codeweaver failed to select files`);
  }
