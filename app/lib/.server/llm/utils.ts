...
export function simplifycodeweaverActions(input: string): string {
  // Using regex to match codeweaverAction tags that have type="file"
  const regex = /(<codeweaverAction[^>]*type="file"[^>]*>)([\s\S]*?)(<\/codeweaverAction>)/g;
...
export function createFilesContext(files: FileMap, useRelativePath?: boolean) {
  ...
  const fileContexts = filePaths
    ...
    .map((path) => {
      ...
      return `<codeweaverAction type="file" filePath="${filePath}">${codeWithLinesNumbers}</codeweaverAction>`;
    });

  return `<codeweaverArtifact id="code-content" title="Code Content" >\n${fileContexts.join('\n')}\n</codeweaverArtifact>`;
}
