...
export class ImportExportService {
    ...
    try {
      ...
      return {
        ...
        core: {
          ...
          codeweaver_user_profile: this._safeGetItem('codeweaver_user_profile'),
          codeweaver_settings: this._safeGetItem('codeweaver_settings'),
          codeweaver_profile: this._safeGetItem('codeweaver_profile'),
        ...
        features: {
          // Feature flags
          viewed_features: this._safeGetItem('codeweaver_viewed_features'),
          developer_mode: this._safeGetItem('codeweaver_developer_mode'),
        ...
        ui: {
          // Tab configuration
          codeweaver_tab_configuration: this._safeGetItem('codeweaver_tab_configuration'),
        ...
        debug: {
          ...
          acknowledged_debug_issues: this._safeGetItem('codeweaver_acknowledged_debug_issues'),
          acknowledged_connection_issue: this._safeGetItem('codeweaver_acknowledged_connection_issue'),

          // Error logs
          error_logs: this._safeGetItem('error_logs'),
          codeweaver_read_logs: this._safeGetItem('codeweaver_read_logs'),
        ...
        updates: {
          ...
          last_acknowledged_update: this._safeGetItem('codeweaver_last_acknowledged_version'),
        },
  ...
  static async deleteAllChats(db: IDBDatabase): Promise<void> {
    ...
    localStorage.removeItem('codeweaver_chat_history');
  ...
  private static async _importComprehensiveFormat(data: any): Promise<void> {
    ...
    if (data.ui) {
      ...
      if (data.ui.codeweaver_tab_configuration) {
        try {
          this._safeSetItem('codeweaver_tab_configuration', data.ui.codeweaver_tab_configuration);
        } catch (err) {
    ...
    if (data.debug) {
      ...
      const debugLocalStorageKeys = [
        'codeweaver_acknowledged_debug_issues',
        'codeweaver_acknowledged_connection_issue',
        'error_logs',
        'codeweaver_read_logs',
      ];
    ...
    if (data.updates) {
      ...
      if (data.updates.last_acknowledged_update) {
        try {
          this._safeSetItem('codeweaver_last_acknowledged_version', data.updates.last_acknowledged_update);
        } catch (err) {
