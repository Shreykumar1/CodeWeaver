...
export class codeweaverShell {
  ...
  async init(webcontainer: WebContainer, terminal: ITerminal) {
    ...
    const { process, commandStream, expoUrlStream } = await this.newcodeweaverShellProcess(webcontainer, terminal);
  ...
  async newcodeweaverShellProcess(webcontainer: WebContainer, terminal: ITerminal) {
    const args: string[] = [];
    const process = await webcontainer.spawn('/bin/jsh', ['--osc', ...args], {
      terminal: {
        cols: terminal.cols ?? 80,
...
export function newcodeweaverShellProcess() {
  return new codeweaverShell();
}
